...
var AlertPool = map[string]Alert{
	fmt.Sprintf("%d-%s-%s", model.Windows, model.LabelChargeStatus, "Fully Charged"): New(model.Toast).Title("Charging Status").Message("Charging Status is Fully Charged, Please disconnect charger."),
	fmt.Sprintf("%d-%s-%s", model.Windows, model.LabelBatteryQuantity, "100%"):       New(model.Toast).Title("Battery Level").Message("Battery Level is 100%, Please disconnect charger."),
	fmt.Sprintf("%d-%s-%s", model.Windows, model.LabelBatteryQuantity, "50%"):        New(model.Toast).Title("Battery Level").Message("Battery Level is 50%."),
	fmt.Sprintf("%d-%s-%s", model.Windows, model.LabelBatteryQuantity, "10%"):        New(model.Toast).Title("Battery Level").Message("Low Battery Level. Please connect charger"),
	fmt.Sprintf("%d-%s-%s", model.Windows, model.LabelBatteryQuantity, "5%"):         New(model.Toast).Title("Battery About to die").Message("Battery Level critical. Please connect charger immediately"),
